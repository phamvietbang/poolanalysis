...
export async function fetchTokensPrice() {
  ...
  const busdContract = new mainnetWeb3Reader.eth.Contract(
    BEP20.abi,
    busd.underlyingAddress
  );
  // const oraiContract = new mainnetWeb3Reader.eth.Contract(BEP20.abi, orai.underlyingAddress);
  const travaContract = new mainnetWeb3Reader.eth.Contract(
    BEP20.abi,
    trava.underlyingAddress
  );

  const WBNB_ADDRESS = "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c";
  const wbnbContract = new mainnetWeb3Reader.eth.Contract(
    WBNB.abi,
    WBNB_ADDRESS
  );

  const wbnbBusdLpAddress = "0x1B96B92314C44b159149f7E0303511fB2Fc4774f";
  const wbnbBalanceBusdlp = await wbnbContract.methods
    .balanceOf(wbnbBusdLpAddress)
    .call();
  const busdBalanceBusdlp = await busdContract.methods
    .balanceOf(wbnbBusdLpAddress)
    .call();
  const bnbPrice = busdBalanceBusdlp / wbnbBalanceBusdlp;

  const wbnbTravaLpAddress = "0x865c77d4ff6383e06c58350a2cfb95cca2c0f056";
  const wbnbBalanceTravalp = await wbnbContract.methods
    .balanceOf(wbnbTravaLpAddress)
    .call();
  const travaBalanceTravalp = await travaContract.methods
    .balanceOf(wbnbTravaLpAddress)
    .call();
  ...
  const travaOnFTMCR = new ftmMainnetWeb3Reader.eth.Contract(
    BEP20.abi,
    travaOnFTM.underlyingAddress
  );
  const usdcCR = new ftmMainnetWeb3Reader.eth.Contract(
    BEP20.abi,
    "0x04068da6c83afcfa0e13ba15a6696662335d5b75"
  );

  const ftmBalance =
    (await wftmCR.methods.balanceOf(FTMnUSDC_ADDRESS).call()) / 1e18;
  const usdcBalance =
    (await usdcCR.methods.balanceOf(FTMnUSDC_ADDRESS).call()) / 1e6;
  const ftmPrice = usdcBalance / ftmBalance;

  const travaBalance = await travaOnFTMCR.methods
    .balanceOf(TRAVAnFTM_ADDRESS)
    .call();
  ...
  const travaPrice =
    getCurrentNetwork().chainId === NETWORKS.BSC_MAINNET.chainId
      ? travaPriceOnBSC
      : travaPriceOnFTM;
